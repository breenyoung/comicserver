...
def _apply_security_scopes(query: SqlQuery, model, user: CurrentUser, allowed_ids: List[int]) -> SqlQuery:
    """
    Applies both Library RLS and Age Rating restrictions dynamically.
    """
...
async def get_search_suggestions(
    ...
    if field == 'series':
        ...
        base = db.query(PullList.name).filter(PullList.name.ilike(f"%{q_str}%"), PullList.user_id == current_user.id)
...
async def quick_search(
    ...
    # 6. Pull Lists
    base_pull = db.query(PullList).filter(PullList.name.ilike(q_str), PullList.user_id == current_user.id)
